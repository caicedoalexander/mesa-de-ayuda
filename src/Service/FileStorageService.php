 ...
 *
 * REFACTORED (TRAIT-002): Extracted from GenericAttachmentTrait (830 lines).
 *
 * Handles file upload, download, and deletion for all entity types
 * (Tickets, PQRS, Compras) with S3 and local storage support.
 *
 * Responsibilities:
 * - File validation (type, size, MIME, security)
 * - S3 and local file storage
 * - Attachment database records
 * - File URL generation (local + S3 presigned)
...
class FileStorageService
{
     ...
     * Constructor
     *
     * @param S3Service $s3Service S3 storage service (injected)
     */
    ...
    public function saveUploadedFile(
        ...
        if ($useS3) {
            ...
            try {
                ...
                // Upload to S3
                if (!$this->s3Service->uploadFile($tempPath, $s3Key, $mimeType)) {
     ...
     * @return EntityInterface|null Attachment entity or null
     */
    public function saveFromBinary(
        string $entityType,
        EntityInterface $entity,
        string $filename,
     ...
     */
    public function deleteAttachment(string $entityType, int $attachmentId): bool
    {
        try {
            $attachmentTableName = $this->getAttachmentTableName($entityType);
            $attachmentsTable = $this->fetchTable($attachmentTableName);
            ...
            if ($this->s3Service->isEnabled()) {
                $this->s3Service->deleteFile($attachment->file_path);
            } else {
    ...
    public function getFullPath(EntityInterface $attachment): ?string
    {
        if ($this->s3Service->isEnabled()) {
            ...
            if ($this->s3Service->downloadFile($attachment->file_path, $tempPath)) {
                return $tempPath;
            }
    ...
    {
        if ($this->s3Service->isEnabled()) {
            return $this->s3Service->getPresignedUrl($attachment->file_path, 60);
        }
    ...
    {
        if ($this->s3Service->isEnabled()) {
            return $this->s3Service->getFileStream($attachment->file_path);
        }
     ...
     */
    public function getEntityNumber(string $entityType, EntityInterface $entity): string
    {
