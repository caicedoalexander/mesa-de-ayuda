...
class SettingsController extends AppController
{
     ...
     * SLA Configuration
     *
     * Allows admin to configure SLA (Service Level Agreement) settings
     * for both PQRS (type-based) and Compras (single) modules.
     *
     * @return \Cake\Http\Response|null|void
     */
    public function sla()
    {
        if ($this->request->is('post')) {
            ...
            // Save all SLA settings
            $saved = 0;
            foreach ($data as $key => $value) {
                if (str_starts_with($key, 'sla_')) {
                    if ($this->_saveSetting($key, $value)) {
                        $saved++;
                    }
                }
            }
            ...
            if ($saved > 0) {
                $this->Flash->success(__('Configuración de SLA guardada exitosamente. {0} configuraciones actualizadas.', $saved));
            } else {
                $this->Flash->error(__('Error al guardar la configuración de SLA.'));
            }

            return $this->redirect(['action' => 'sla']);
        }

        // Load current SLA settings
        $allSettings = $this->_loadSettings();

        // Filter SLA settings
        $slaSettings = [];
        foreach ($allSettings as $key => $value) {
            if (str_starts_with($key, 'sla_')) {
                $slaSettings[$key] = $value;
            }
        }

        $this->set(compact('slaSettings'));
    }
}
